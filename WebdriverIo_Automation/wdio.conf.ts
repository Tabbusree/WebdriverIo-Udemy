...
export const config: Options.Testrunner = {
   ...
   * binary=<location>
   *--auth-server-whitelist="_"  // forcefully invoke the user login
   */
  capabilities: [
    {
      maxInstances: 5,
  ...
  reporters: [
    ...
    [
      "allure",
      {
        ...
        disableWebdriverStepsRecording: true,
  ...
  before: async function (/*capabilities, specs*/) {
    // require("ts-node").register({ files: true });  //not applicable ESmodule
    await import("ts-node").then((tsNode) => tsNode.register({ files: true }));
  },
  /**
  ...
  beforeScenario: function (world) {
    ...
    let scenarioName = world.pickle.name;
    console.log(scenarioName);
    // Split the scenario name by ':'
    ...
    // Throw an error if testid is not set
    //@ts-ignore
    if (!browser.options.testid) {
      throw new Error(
        `Error getting testid for current scenario: ${scenarioName}`
      );
    }
    // Log the testid to the console for debugging
    //@ts-ignore
    console.log(`>> Test id: ${browser.options.testid}`);
  },
  /**
   *
  ...
  beforeStep: function (context) {
    ...
    console.log(`>>Before step testid: ${this.testid}`);
  },
  /**
   *
